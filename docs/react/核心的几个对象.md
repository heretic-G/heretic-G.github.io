#### ReactElement 由ReactDom.createElement创建的对象

```flow js
type ReactElement = {|
  // 类型
  $$typeof: any,
  // 这里决定了不同的逻辑 string就是原生的 fun和class都是传入的方法
  type: any,
  key: any,
  ref: any,
  props: any,
...
this.stateNode = null; // 跟当前Fiber相关本地状态（比如浏览器环境就是DOM节点） 函数组件指向函数 class组件指向class
