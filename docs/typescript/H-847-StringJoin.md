...
type strJoin<A extends string[], S extends string> = A extends [infer F, ...infer Rest] ? `${F & string}${S}${strJoin<Rest extends string[] ? Rest : [], S>}` : ''

declare function join<T extends string>(delimiter: T): <P extends string[]>(...parts: P) => strJoin<P, T> extends `${infer F}${T}` ? F : ''
